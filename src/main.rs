...
lazy_static! {
    static ref SERVER_KEY: Arc<Mutex<Option<tfhe::ServerKey>>> = Arc::new(Mutex::new(None));
...
fn init_server_key(server_key: tfhe::ServerKey) {
    INIT.call_once(|| {
        let mut server_key_lock = SERVER_KEY.lock().unwrap();
        *server_key_lock = Some(server_key);
    });
}
...
{
    // println!("Running operation: {}", operation_name);
    // // Ensure the server key is set in the thread context
    // let server_key = SERVER_KEY.lock().unwrap().clone().expect("Server key not set");
    ...
    let result = panic::catch_unwind(|| {
        println!("Trying to set server key...");
        if (SERVER_KEY.lock().unwrap()).is_none() {
            println!("Server key not set, setting it now...");
        } else {
            println!("Server key already set...");
        }
        let server_key = SERVER_KEY.lock().unwrap().clone().expect("Server key not set");
        set_server_key(server_key);
        println!("Server key set successfully");
        ...
        let start = Instant::now();
        // let num1 = encrypted_num1.lock().unwrap().clone();
        // let num2 = encrypted_num2.lock().unwrap().clone();
        let result = operation(&encrypted_num1, &encrypted_num2);
