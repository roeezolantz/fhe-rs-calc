...
// Define a struct to hold both keys
struct Keys {
...
lazy_static! {
    static ref KEYS: Keys = generate_keys_struct();
}
...
fn generate_keys_struct() -> Keys {
    let config = ConfigBuilder::default().build();
    let (client_key, server_key) = generate_keys(config);
    Keys { client_key, server_key }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    init_server_key();
    let client_key = KEYS.client_key.clone();

    // Perform operations on various FHE types
    perform_operation::<u8, FheUint8>(20, 10, &client_key);
    perform_operation::<u16, FheUint16>(200, 100, &client_key);
