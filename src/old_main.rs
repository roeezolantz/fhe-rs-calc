...
// Helper function to perform addition on an arbitrary length of ciphertexts
fn add(encrypted_nums: &[FheUint16]) -> FheUint16 {
    encrypted_nums.iter().skip(1).fold(encrypted_nums[0].clone(), |acc, x| &acc + x)
}